...
# iterate over possible dfs
dfs = c('cutie_df_abun_saliva')

for (y in c('Stool', 'Saliva')){
  if (y == 'Stool'){
  ...
  for(l in 1:length(dfs)){
    metadf = dfs[l]
    metadata = read.table(file=paste('inputs/',metadf,'.tsv',sep=''),sep='\t',header=T, row.names=1, stringsAsFactors=F, check.names=F,comment.char='@')
    ...
    file_path <- paste('/Users/KevinBu/Desktop/clemente_lab/Projects/oa/outputs/jobs17/heatmap_corr_',metadf,'_',y,'.pdf',sep='')
